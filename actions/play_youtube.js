...
<div>
    ...
    action: async function (cache) {
        ...
        const getInfoAsync = WrexMods.require('util').promisify(ytdl.getInfo);
        const msg = cache.msg;
        const options = {};
        if (data.seek) {
        ...
        const song = await getInfoAsync(url)
            .catch((err) => {
                console.log(`Error with getInfoAsync in play_youtube: ${err}`);
            });
		options.title = song.title;
		options.duration = parseInt(song.length_seconds);
		options.thumbnail = song.player_response.videoDetails.thumbnail.thumbnails[3].url;
        if (url) {
